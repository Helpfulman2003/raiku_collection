...
const DragonEffect2D = () => {
  ...
  useEffect(() => {
    ...
    const prepend = (use, i) => {
      ...
      elems[i] = { use: elem, x: width / 2, y: 0 };
      elem.setAttributeNS(xlinkns, "href", "#" + use);
      screen.prepend(elem);
    };

    // Initialize elements array with empty objects first
    for (let i = 0; i < N; i++) {
    ...
    const run = () => {
      ...
      let e = elems[0]; // Wait, if prepend only adds from 1 to N-1, elems[0] use is null.
      // The original code had: for (let i = 0; i < N; i++) elems[i] = { use: null, x: width / 2, y: 0 };
      // And then for (let i = 1; i < N; i++) prepend(...) which replaced elems[i].use. So elems[0].use remains null.
  ...
  return (
    <svg id="dragon-svg" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', pointerEvents: 'none', zIndex: 101 }}>
      <defs>
        <g id="Cabeza" transform="matrix(1, 0, 0, 1, 0, 0)">
          <path style={{ fill: '#bfff00', fillOpacity: 1 }} d="M-28.9,-1.1L-28.55 -1.95Q-28.1 -3.1 -27.25 -2.95L-26.7 -2.95Q-27.7 -1.65 -28.9 -1.1M-18.35,-1.8Q-15.1 -10.3 -9.6 -6.05Q-15.1 -6.2 -18.35 -1.8M-18.35,1.1Q-15.1 5.45 -9.6 5.35Q-15.1 9.55 -18.35 1.1M-26.7,2.2L-27.25 2.25Q-28.1 2.4 -28.55 1.2L-28.9 0.35Q-27.7 0.9 -26.7 2.2" />
        ...
        <g id="Aletas" transform="matrix(1, 0, 0, 1, 0, 0)">
          <linearGradient id="LinearGradID_1" gradientUnits="userSpaceOnUse" gradientTransform="matrix(0.0935974, 0, 0, 0.188782, -20.55, 0)" spreadMethod="pad" x1="-819.2" y1="0" x2="819.2" y2="0">
            <stop offset="0" style={{ stopColor: '#bfff00', stopOpacity: 1 }} />
